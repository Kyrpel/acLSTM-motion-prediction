...
class acLSTM(nn.Module):
    ...
    def cal_loss(self, out_seq, groundtruth_seq):
        ...
        # Seq_len = out_seq.shape[1] // 175

        output = out_seq.reshape(32, Seq_len, 175)
        hip_output = output[:, :, 0:3].reshape(32, Seq_len*3)
        euler_output = output[:, :, 3:].reshape(32*Seq_len*258)


        gt = groundtruth_seq.reshape(32, Seq_len, 175)
    ...
    def calculate_quaternion_loss(self, out_seq, groundtruth_seq):
        ...
        out_seq = out_seq.reshape(32, Seq_len, 175)
        hip_output = out_seq[:, :, 0:3].reshape(-1, 3)
        euler_output = out_seq[:, :, 3:].reshape(-1, 4)  # Assuming the rest of the sequence are quaternions

        groundtruth_seq = groundtruth_seq.reshape(32, Seq_len, 175)
        hip_gt = groundtruth_seq[:, :, 0:3].reshape(-1, 3)
        euler_gt = groundtruth_seq[:, :, 3:].reshape(-1, 4)  # Assuming the rest of the sequence are quaternions

        # Normalize quaternions
        euler_output = F.normalize(euler_output, p=2, dim=-1)
        euler_gt = F.normalize(euler_gt, p=2, dim=-1)

        # Compute the dot product along the last dimension (equivalent to cosine of angle for normalized quaternions)
        dot_product = (euler_output * euler_gt).sum(-1)

        # Clamp values for numerical stability,  good practice to use 0.999 values  to handle and  prevent NaN values in computations involving inverse trigonometric functions.
        ...
        # Compute the loss
        qloss = 2 * torch.acos(abs_dot_product).mean()

        # mse for hip
        hip_loss = nn.MSELoss()
        hip_mse = hip_loss(hip_output, hip_gt)

        print("hip loss: ", hip_mse)
        print("quaternion loss: ", qloss)

        total_loss = hip_mse + qloss
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False, save_bvh_motion=True):
    ...
    # check for nan values on the predicted sequence
    print(f'predict_seq is nan: {torch.isnan(predict_seq).any()}')
    print(f'predict_groundtruth_seq is nan: {torch.isnan(predict_groundtruth_seq).any()}')
    print(f' is inf: {torch.isinf(predict_seq).any()}')
    print(f' is inf: {torch.isinf(predict_groundtruth_seq).any()}')
    ...
    if(print_loss==True):
        print ("###########"+"Quart iter %07d"%iteration +"######################")
...
if os.path.exists("/kaggle/input/auto-rnn"):#if we are running on kaggle
    ...
    write_weight_folder="kaggle/working/weights/weights_quar/"
    # location to save the temporate output of the network and the groundtruth motion sequences in the form of bvh
    write_bvh_motion_folder="kaggle/working/quar_train_tmp_bvh_aclstm_martial/"
    # location of the training data
    dances_folder = "/kaggle/input/auto-rnn/train_data/quar_train_xyz/"
    ...
    read_weight_path=""
    # location to save the weights of the network during training
    write_weight_folder="./weights/weights_quar/"
    # location to save the temporate output of the network and the groundtruth motion sequences in the form of bvh
    write_bvh_motion_folder="./tmp/quar_train_tmp_bvh_aclstm_martial/"
    # location of the training data
    dances_folder = "./train_data/quartentions_train_xyz/"
...
train(dances, dance_frame_rate, batch, 100, read_weight_path, write_weight_folder, write_bvh_motion_folder, 15000)
