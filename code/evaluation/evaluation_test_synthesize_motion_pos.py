...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    # print(f'generate_seq, initial_seq_np.shape is {initial_seq_np.shape}') #(5, 30, 171)
    ...
    for b in range(batch):
        ...
        out_seq=np.array(predict_seq[b].data.tolist()).reshape(-1,171)  #(429, 171)
    ...
    return np.array(predict_seq.data.tolist()).reshape(batch, -1, 171)
...
def test(dance_batch_np, frame_rate, batch, initial_seq_len, generate_frames_number, read_weight_path, write_bvh_motion_folder):
    ...
    for b in range(batch):
        ...
        read_bvh.write_traindata_to_bvh(write_bvh_motion_folder+"original"+"%02d"%b+".bvh", np.array(sample_seq))
...
# positional
read_weight_path = "./weights/weights_pos/0006000.weight"
