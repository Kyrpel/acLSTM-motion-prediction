...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    # print(f'generate_seq, initial_seq_np.shape is {initial_seq_np.shape}') #(5, 30, 175)
    ...
    for b in range(batch):
        ...
        out_seq=np.array(predict_seq[b].data.tolist()).reshape(-1,175)  
        ...
        read_bvh.write_quaternion_traindata_to_bvh(save_dance_folder+"15000out"+"%02d"%b+".bvh", out_seq)
    return np.array(predict_seq.data.tolist()).reshape(batch, -1, 175)
...
def test(dance_batch_np, frame_rate, batch, initial_seq_len, generate_frames_number, read_weight_path, write_bvh_motion_folder):
    ...
    for b in range(batch):
        ...
        read_bvh.write_quaternion_traindata_to_bvh(write_bvh_motion_folder+"15000original"+"%02d"%b+".bvh", np.array(sample_seq))
...
# euler
read_weight_path = "./weights/weights_quar/0014000.weight"
write_bvh_motion_folder = "./synthetic_motion/quat/"
