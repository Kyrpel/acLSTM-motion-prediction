...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    # print(f'generate_seq, initial_seq_np.shape is {initial_seq_np.shape}') #(5, 30, 132)
    ...
    for b in range(batch):
        ...
        out_seq=np.array(predict_seq[b].data.tolist()).reshape(-1,132)  #(429, 132)
        ...
        for frame in range(out_seq.shape[0]):
            ...
            # original batch must be the 
            # original_out = initial_seq
            
        read_bvh.write_euler_traindata_to_bvh(save_dance_folder+"15000out"+"%02d"%b+".bvh", out_seq)
    return np.array(predict_seq.data.tolist()).reshape(batch, -1, 132)
...
def test(dance_batch_np, frame_rate, batch, initial_seq_len, generate_frames_number, read_weight_path, write_bvh_motion_folder):
    ...
    for b in range(batch):
        ...
        start_id=random.randint(10, int(dance_len-(initial_seq_len+generate_frames_number)*speed-10))#the first and last several frames are sometimes noisy. 
        sample_seq=[]
        for i in range(initial_seq_len+generate_frames_number): # extract sequence of initial_seq_len+generate_frames_number frames
            sample_seq=sample_seq+[dance[int(i*speed+start_id)]]
        
        # save the original sequence
        read_bvh.write_euler_traindata_to_bvh(write_bvh_motion_folder+"15000original"+"%02d"%b+".bvh", np.array(sample_seq))

        # only take initial_seq_len frames for prediction
        dance_batch=dance_batch+[sample_seq[:initial_seq_len]]
...
# # euler
# read_weight_path = "./weights/weights_euler/0015000.weight"
# write_bvh_motion_folder = "./synthetic_motion/euler/"
# dances_folder = "./train_data/euler_train_xyz/"

read_weight_path = "./weights/weights_euler/weight_loss_weights/0006000.weight"
write_bvh_motion_folder = "./synthetic_motion/euler/weight_euler/"
dances_folder = "./train_data/euler_train_xyz/"
